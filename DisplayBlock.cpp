...
int PlayerDB::playerLogin(ENetPeer* peer, string username, string password) {
	...
	if (ifs.is_open()) {
		...
		else if (((PlayerInfo*)(peer->data))->gameversion != "2.988") {

			string text = "action|log\nmsg|`4UPDATE REQUIRED`o : The `$V2.988 `oupdate is now avallable for your device. Go get it! You'll need that before you can login private server.\n";
			string text3 = "action|logon_fail\n";
...
void updateDisplay(ENetPeer* peer, int x, int y, int id)
{
		...
		++currentPeer)
	{
		...
		if (isHere(peer, currentPeer)) {
			...
			data.blockid = 2946; // 2946 3794 = display shelf
								 //data.netID = ((PlayerInfo*)(peer->data))->netID;
			data.backgroundid = 664;
			data.visual = 9x8000000; //0x00210000
