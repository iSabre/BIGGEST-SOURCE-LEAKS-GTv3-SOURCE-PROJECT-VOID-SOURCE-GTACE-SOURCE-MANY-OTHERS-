...
struct PlayerInfo {
	...
	bool boughtFYE = false; // phoenix sword
	bool boughtAAC = false; // ancestral items
	bool boughtRFS = false; // Raymans fists
	bool boughtGRN = false; // Gemini ring
	bool boughtWRN = false; // Water ring
	bool boughtWSD = false; // Winds ring
	bool boughtDAV = false; // Da vinci Wings
	bool boughtTWS = false; // Twin Swords
	bool boughtZUS = false; // Zeus bolt
	bool boughtMEG = false; // Megeladon demon
	bool boughtWCL = false; // Water cloak
	bool boughtBUR = false; // Blue aura
	bool boughtTK6 = false; // Tk69 Board
	bool boughtFCS = false; // focus eye
	bool boughtGPK = false; // gold pick
	bool boughtPES = false; // puppy leash
	bool isNicked = false;
	string tankIDName = "";
	string tankIDPass = "";
...
int PlayerDB::playerLogin(ENetPeer* peer, string username, string password) {
	...
	if (ifs.is_open()) {
		...
		if (ban == 1) {
			...
			string text = "action|log\nmsg|`4UPDATE REQUIRED`o : The `$V2.999 `oupdate is now avallable for your device. Go get it! You'll need that before you can login private server.\n";
			string text3 = "action|logon_fail\n";
...
void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
{
	...
	if (!isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))
	{
		...
		{
			GamePacket p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`wIt's too strong to place``"), 0));
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	*/
	...
	cout << "GrowtopiaFX (C) Zedd + Valkrie 2019" << endl;
	enet_initialize();
			...
			case ENET_EVENT_TYPE_RECEIVE:
			{
				...
				case 2:
				{
					...
					if (cch.find("action|store") == 0)
						if (((PlayerInfo*)(peer->data))->haveGrowId == true)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label|small|`7Welcome to the `2GTFX `wStore!|left|\nadd_smalltext|`wTap the item you'd like to purchase. want any `9Gems`w/`1Levels`w? `eVIP `wStatus? `#@Moderator `wStatus? Click on ' `wPurchase `4in-game Assets`w '.|left|\nadd_spacer|small|\nadd_button|items|`wPurchase `9in-game Items|\nadd_button|assets|`wPurchase `4in-game Assets|\nadd_button|pxps|`wPurchase `3XP BOOST|\nadd_spacer|small|\nadd_button|edrink|`wPurchase `2Energy Drinks|\nadd_button|mplace|`wMarketPlace|\nadd_button|prewl|`4GTFX: `9Premium WLS|\nadd_spacer|small||\nend_dialog|chc0|Cancel|\n"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
					...
					if (cch.find("action|dialog_return") == 0)
					{
						...
						if (btn == "assets") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wWelcome to our Shop!``|left|1430|\n\nadd_spacer|small|\nadd_label|small|`wPurchase `9Gems `w- `32000/1World Lock|left|\nadd_label|small|`wPurchase `1Levels `w- `310/1World Lock|left|\nadd_spacer|small|\nadd_label|small|`wPurchase `eVIP `w- `35Diamond Locks|left|\nadd_label|small|`wPurchase `#MOD `w- `315Diamond Locks|left|\nadd_spacer|small|\nadd_label|small|`wPurchase `bRank `wButtons|left|\nadd_spacer|small|\nadd_button|vips|`wPurchase `eVIP|0|0|\nadd_button|mods|`wPurchase `#MOD|0|0|\nadd_spacer|small|\nend_dialog|chc0|Close|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "edrink") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase `2Energy Drinks|left|3306|\nadd_spacer|small|\nadd_label|small|`wTo Purchase a `2Energy Drink`w, Simply Click on '`wPurchase `2Drink!`w', Once you have clicked on it you will be gived a random buff such as; Gem Boost, XP Boost, etc.|left|\nadd_spacer|small|\nadd_label_with_icon|small|`9Purchase Energy Drinks`2:|left|1602|\nadd_spacer|small|\nadd_button_with_icon|yesedrink||staticBlueFrame|1662|2000|\nadd_button_with_icon||END_LIST|noflags|0|0||\nadd_spacer|small|\nadd_button|chc0|Close|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesedrink") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`wSuccessfully Purchased `2Energy Drink`w!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "items") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wWelcome to our store|left|112|\nadd_spacer|small|\nadd_label|small|`rPlease select an item that you'd like to purchase!|left|\nadd_spacer|small|\nadd_label_with_icon|small|`eSpecial Items`2:|left|6312|\nadd_button_with_icon|rfs||staticBlueFrame|5480|35000|\nadd_button_with_icon|aac||staticBlueFrame|5078|150000|\nadd_button_with_icon|fye||staticBlueFrame|6312|500000|\nadd_button_with_icon||END_LIST|noflags|0|0|\nadd_spacer|small|\nadd_button|chc0|Close|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "punishview") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`!Punish/View Player|left|1432|\nadd_spacer|small|\nadd_button|punishplayer|`!Punish `w(`4Ban`!/`4IP Ban`!/`4Kick`w)|0|0|\nadd_button|viewplayer|`!View `w(`4Check IP`!/`4Information`w)|0|0|\nadd_spacer|small|\nend_dialog|chc0|Cancel|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "punishplayer") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`!Punish Player|left|1432|\nadd_spacer|small|\nadd_button|banuser|`4Ban from Game|\nadd_button|ipbanuser|`4IP-Ban from Game|\nadd_button|kickuser|`4Kick|\nadd_spacer|small|\nadd_button|punishview|`wBack|\nadd_button|chc0|`wCancel|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "viewplayer") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`!View Player|left|1432|\nadd_spacer|small|\nadd_label|small|`9Username: " + username + "|left|\nadd_label|small|`9Player IP: `4UNKNOWN?|left|\nadd_label|small|`9Player Platform: IOS / Macbook|left|\nadd_spacer|small|\nadd_label|small|`wMore View options are coming soon! stay Tuned `#@Moderators`w!|left|\nadd_spacer|small|\nend_dialog|punishview|Back|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "banuser")
						{
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 666) {
								...
								GamePacket p7 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `$The Ancient Ones `ohave `4banned `w" + username + " `#** `o(`4/rules `oto see the rules!)"));
								ENetPeer* currentPeer;
									...
									++currentPeer)
								{
									...
									if (((PlayerInfo*)(currentPeer->data))->rawName == ((PlayerInfo*)(peer->data))->lastInfo) {
										...
										GamePacket ps2 = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`0Warning from `4System`0: You've been `4BANNED `0from GrowTopiaFX for 730 days"), "audio/hub_open.wav"), 0));
										ENetPacket* packet2 = enet_packet_create(ps2.data,
											ps2.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet2);
										GamePacket ps3 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`oWarning from `4System`o: You've been `4BANNED `ofrom GrowTopiaFX for 730 days"));
										ENetPacket* packet3 = enet_packet_create(ps3.data,
											ps3.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet3);
										GamePacket pto = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "Applied punishment."));
										//memcpy(p2.data + 8, &(((PlayerInfo*)(currentPeer->data))->netID), 4);
						...
						if (btn == "ipbanuser")
						{
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 666) {
								...
								GamePacket p7 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4** `o" + username + " `4IPBANNED BY MOD **"));
								ENetPeer* currentPeer;
						...
						if (btn == "kickuser")
						{
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 666)
							{
									...
									++currentPeerp)
								{
									...
									{
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`2Successfully `4Kicked `wPlayer from `9GrowtopiaFX"));
										ENetPacket* packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
						...
						if (btn == "mods") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase Moderator|left|18|\nadd_smalltext|`4Make sure to read this information clearly!|\nadd_smalltext|`oPrice: `31500 `9World Locks|\nadd_smalltext|`oDuration: `w[`4~`w]|\nadd_smalltext|`oStock: `w[`4~`w]|\nadd_spacer|small|\nadd_label|small|`6Rules:|left|\nadd_spacer|small|\nadd_smalltext|`e1. `rDo not abuse your role.|\nadd_smalltext|`e2. `rIf you are going to ban people, make sure to have screenshots/video for proof|\nadd_smalltext|`e3. `rSharing account will result in account loss.|\nadd_smalltext|`e4. `rTrying to sell your account will result in ip-ban!|\nadd_spacer|small|\nadd_spacer|small|\nadd_label|small|`6Commands:|left|\nadd_spacer|small|\nadd_smalltext|`eAll commands are displayed in /ahelp (Admin help)|\nadd_spacer|small|\nadd_spacer|small|\nadd_button|yesmods|`wPurchase for `91500 Wls|\nadd_button|chc0|`wClose|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "vips") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase VIP|left|18|\nadd_smalltext|`4Make sure to read this information clearly!|\nadd_smalltext|`oPrice: `3600 `9World Locks|\nadd_smalltext|`oDuration: `w[`4~`w]|\nadd_smalltext|`oStock: `w[`4~`w]|\nadd_spacer|small|\nadd_label|small|`6Rules:|left|\nadd_spacer|small|\nadd_smalltext|`e1. `rDo not spam with VIP-SB|\nadd_smalltext|`e2. `rImpersonation someone with name changing will result in ban!|\nadd_smalltext|`e3. `rSharing account will result in account loss.|\nadd_smalltext|`e4. `rTrying to sell your account will result in ip-ban!|\nadd_spacer|small|\nadd_spacer|small|\nadd_label|small|`6Commands:|left|\nadd_spacer|small|\nadd_smalltext|`eAll commands are displayed in /vhelp (vip help)|\nadd_spacer|small|\nadd_spacer|small|\nadd_button|yesvips|`wPurchase for `9500 Wls|\nadd_button|chc0|`wClose|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "prewl") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`7Purchase Server WLS|left|242|\nadd_spacer|small|\nadd_spacer|small|\nadd_label|small|`6Deposit WLS in real Growtopia World: `7SOLO45|left|\nadd_spacer|small|\nadd_label|small|`9Put how many WLS you'd like to purchase, 1GTWL = 1FXWL, 1GTDL = 100FXWLS. `2+ for donation note add your private server GrowID!|left|\nadd_label|small|`5Cick below to see what you can purchase with Server WLS!|left|\nadd_spacer|small|\nadd_button|swls|Spend WLS|\nadd_spacer|small|\nend_dialog|chc0|Thank you|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "swls") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wYou have `20 `wWorld Locks!|left|242|\nadd_spacer|small|\nadd_label|small|`rPlease select items that you'd like to purchase.|left|\nadd_spacer|small|\nadd_button_with_icon|ranks|`9Purchase Rank|noflags|408|\nadd_button_with_icon|pgems|`2Purchase Gems|noflags|112|\nadd_button_with_icon|pxps|`5Purchase XP|noflags|1488|\nadd_button_with_icon||END_LIST|noflags|0|0||\nadd_spacer|small|\nadd_button|purchasewl|`wPurchase `9World Locks|\nend_dialog|chc0|Close|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "ranks") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase `#MOD`w/`eVIP|left|408|\nadd_spacer|small|\nadd_label|small|`rPlease choose the item you'd like to purchase!|left|\nadd_spacer|small|\nadd_button_with_icon|mods|`#MOD `w(`91500Wls`w)|noflags|278|\nadd_button_with_icon|vips|`eVIP `w(`9500Wls`w)|noflags|158|\nadd_button_with_icon||END_LIST|noflags|0|0||\nadd_spacer|small|\nadd_button|swls|Back|\nadd_button|chc0|Close|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "pgems") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wPurchase Gems|left|112|\nadd_spacer|small|\nadd_label|small|`91WL `1Is 2000 Gems!|\nadd_label|small|`9You have `20 `9WLS, how much you want to spend WLS for Gems? `1(Enter WLS Ammount):|left|\nadd_spacer|small|\nadd_text_input|aogems|`wAmmount of `9WLS:||75|\nadd_spacer|small|\nend_dialog|chc0|Cancel|`5Check Price|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "pxps") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`1Purchase XP|left|1488|\nadd_spacer|small|\nadd_label|small|`9You have `20 `9WLS, how much you want to spend WLS for XP?|left|\nadd_spacer|small|\nadd_label|small|`1Choose which potion you'd like to purchase:|left|\nadd_spacer|small|\nadd_button_with_icon|yesxp2||staticBlueFrame|1488|100|\nadd_button_with_icon|yesxp2||staticBlueFrame|1488|500|\nadd_button_with_icon|yesxp2||staticBlueFrame|1488|1250|\nadd_button_with_icon||END_LIST|noflags|0|0||\nadd_spacer|small|\nadd_button|swls|`wBack|\nadd_button|chc0|`wClose|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "mplace") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wMarketPlace|left|1434|\nadd_spacer|small|\nadd_label|small|`oWelcome `2" + username + "`o, To the `wMarketPlace!|left|\nadd_label|small|`oHere you can buy or sell items globally|left|\nadd_spacer|small|\nadd_button|britems|`wSearch Items|\nadd_spacer|small|\nadd_label|small|`wThere are currently `o0 `wItems listed for sale.|left|\nadd_spacer|small|\nadd_button|slitem|`wSell an item|\nadd_spacer|small|\nadd_label|small|`eMarket Status:|left|\nadd_spacer|small|\nadd_label|small|`wItems Sold: `20|left|\nadd_label|small|`wItems Purchased: `20|left|\nadd_label|small|`wListed Items: `20|left|\nadd_spacer|small|\nadd_button|slitem|`wYour Listed Items|\nadd_spacer|small|\nadd_label|small|`wYou currently have `20 GTFX WLS `w.|left|\nadd_spacer|small|\nadd_button|chc0|Thank You|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "britems") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wSearch Items For-Sale|left|3802|\nadd_spacer|small|\nadd_textbox|`rNo one is selling Ancestral items..|\nadd_textbox|`rNo one is selling Phoenix sword..|\nadd_textbox|`rNo one is selling Rayman's Fists..|\nadd_spacer|small|\nadd_label|small|Enter player name that you want to purchase items from!|left|\nadd_spacer|small|\nadd_text_input|pname2|`3Player Name:||65|\nadd_spacer|small|\nend_dialog|chc0|Cancel|Purchase|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "slitem") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wMarket Settings|left|2586|\nadd_spacer|small|`wWelcome `2" + username + "`w, Control your market Settings in here by disable items for-sale / Price changing.|left|\nadd_spacer|small|\nadd_label_with_icon|small|`wAdd in your Market`2:|left|32|\nadd_spacer|small|\nadd_label|small|`o>> You don't have anything listed for-sale|left|\nadd_spacer|small|\nadd_label_with_icon|small|`wEdit Market`4:|left|32|\nadd_spacer|small|\nadd_label|small|`o>> You don't sell anything!|left|\nadd_spacer|small|\nadd_spacer|small|\nend_dialog|chc0|Thank You|"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "fye")
						{
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wPhoenix Sword``|left|6312|\nadd_spacer|small|\nadd_label|small|`rMake sure this is the correct item!|left|23|\nadd_spacer|small| |left|21|\nadd_label|small|`4You are about to purchase `wPhoenix Sword!|left|\nadd_spacer|small|\nadd_label|small|`oThis item contains: `8Phoenix Sword `b+ `2/Ssb `oCommand!|left|\nadd_spacer|small|\nadd_button|yesfye|`9Purchase for `w- `e500,000`q$|noflags|0|0|\nadd_button|no|Nevermind|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wRefund Phoenix Sword`|left|6312|\nadd_spacer|small|\nadd_label|small|`rMake sure this is the correct item!|left|\nadd_spacer|small|\nadd_label|small|`4You are about to refund `wPhoenix Sword!|left|\nadd_spacer|small|\nadd_label|small|`oThis item contains:`4NONE|left|\nadd_spacer|small|\nadd_button|refundfye|`4Refund This item!|noflags|0|0|\nadd_button|no|Nevermind|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "aac")
						{
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wAncestral Items``|left|5078|\nadd_spacer|small|\nadd_label|small|`rMake sure this is the correct item!|left|23|\nadd_spacer|small| |left|21|\nadd_label|small|`4You are about to purchase `wAncestral Items!|left|\nadd_spacer|small|\nadd_label|small|`oThis item contains: `8Ancestral Items|left|\nadd_spacer|small|\nadd_button|yesanc|`9Purchase for `w- `e150,000`q$|noflags|0|0|\nadd_button|no|Nevermind|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`wRefund Ancestral Items|left|5078|\nadd_spacer|small|\nadd_label|small|`rMake sure this is the correct item!|left|23|\nadd_spacer|small| |left|21|\nadd_label|small|`4You are about to refund `wAncestral Items!|left|23|\nadd_spacer|small|\nadd_label|small|`oThis item contains:`4NONE|left|\nadd_spacer|small|\nadd_button|refundaac|`4Refund This item!|noflags|0|0|\nadd_button|no|Nevermind|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "rfs")
						{
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|`eCool Items``|left|5480|\nadd_spacer|small|\nadd_label|small|`rMake sure this is the correct item!|left|\nadd_spacer|small|\nadd_label|small|`4You are about to purchase `wRayman's Fists!|left|\nadd_spacer|small|\nadd_label|small|`oThis item contains: `3Rayman's Fists|left|\nadd_spacer|small|\nadd_button|yesrfs|`9Purchase for `w- `e35,000`q$|noflags|0|0|\nadd_spacer|small|\nadd_button|no|Nevermind|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesfye") {
							if (((PlayerInfo*)(peer->data))->gem > 499999)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`wSuccessfully `5Purchasing `8Phoenix Sword"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesanc") {
							...
							if (((PlayerInfo*)(peer->data))->gem > 149999)
							{

								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`wSuccessfully `5Purchasing `2Ancestral"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`4TRANSACTION FAILED! - `wYou need to have enough gems to buy `6Ancestral`o!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "yesrfs") {
							...
							if (((PlayerInfo*)(peer->data))->gem > 34999)
							{

								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`wSuccessfully `5Purchasing `3Rayman's Fists"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
								...
								((PlayerInfo*)(peer->data))->gem = ((PlayerInfo*)(peer->data))->gem - 35000;
								GamePacket psa = packetEnd(appendInt(appendString(createPacket(), "OnSetBux"), ((PlayerInfo*)(peer->data))->gem));
								ENetPacket* packetsa = enet_packet_create(psa.data, psa.len, ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packetsa);
						...
						if (btn == "refundaac") {
							if (((PlayerInfo*)(peer->data))->gem > 0)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`2Successfully `4Refunded `8Ancestral Items`w!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`4TRANSACTION FAILED! - `wYou need to have enough gems to buy `bDigger spade`w!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "refundrfs") {
							if (((PlayerInfo*)(peer->data))->gem > 0)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`2Successfully `4Refunded `3Rayman's Fists`w!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r`4TRANSACTION FAILED! - `wYou need to have enough gems to buy `3Rayman's Fists`w!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (btn == "refundfye") {
							if (((PlayerInfo*)(peer->data))->gem > 0)
							{
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`2Successfully `4Refunded `8Phoenix Sword`w!"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
					...
					SKIPFind:;
						...
						if (btn == "ringwinds") {
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`9Quest Step Complete"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
					...
					if (cch.find("text|") != std::string::npos) {
						...
						if (str == "/mod") {
							...
							if (((PlayerInfo*)(peer->data))->canWalkInBlocks) {
								p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4/MOD was Disabled"));
								((PlayerInfo*)(peer->data))->canWalkInBlocks = false;
								sendState(peer);
							}
							else {
								p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`2/MOD was Enabled"));
								((PlayerInfo*)(peer->data))->canWalkInBlocks = true;
								sendState(peer);
							}
						...
						else if (str == "/magic")
						{
								...
								++currentPeer)
							{
								...
								if (((PlayerInfo*)(currentPeer->data))->rawName == PlayerDB::getProperName(lvl_user)) {
									...
									delete p67.data;
									GamePacket p0 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`6>> You have sent `2" + lvl_amount + "`6$ To " + ((PlayerInfo*)(currentPeer->data))->Chatname + "`w."));
									ENetPacket* packet0 = enet_packet_create(p0.data,
										p0.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet0);
									...
									delete p68.data;
									GamePacket ps = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`6>> You've received `2" + lvl_amount + "`6$ from " + ((PlayerInfo*)(peer->data))->Chatname + "`6!"));
						...
						else if (str == "/dev")
						{
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`2Ghost mode Enabled! If you want hide name do /nick"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 666 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 333) {
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 && ((PlayerInfo*)(peer->data))->rawName == "hadi")
							{
							...
							{
								name2 = "`w[`eVIP`w] " + ((PlayerInfo*)(peer->data))->tankIDName;
						...
						delete p.data;
							...
							if (!isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`9Owner Commands: `w/help, /ahelp, /vhelp, /ban <user>, /kick <user>, /kickall, /pull <user>, /howgay, /give <gems>, /level <levels>, /xp <amount>, /online, /find, /item <id>, /weather <id>, /sweather <id>, /nick <name>, /spawn <id>, /deposit, /store, /mods, /vips, /mod, /summon <user>, /ghost, /unghost, /asb, /osb, /vsb,"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (Mod(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) break;
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#Mod command : `2/ban, /nuke, /nick, /block, /weather, /msb, /spawn, /find, /mods, /sweather, /invis, /vis, /jsb, /summon "));
						ENetPacket* packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
						...
						else if (str.substr(0, 7) == "/howgay")
						{
								...
								++currentPeer)
							{
								...
								{
									string name = ((PlayerInfo*)(peer->data))->displayName;
									GamePacket p2 = packetEnd(appendIntx(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`2" + name + " `0are `4" + std::to_string(val) + "% `#Gay!"), 0));
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`2" + name + "`0 are `4" + std::to_string(val) + "% `#Gay!"));
									ENetPacket* packet2 = enet_packet_create(p2.data,
										p2.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(currentPeer, 0, packet2);
							...
							if (!canSB(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) continue;
							cout << "OSB from " << ((PlayerInfo*)(peer->data))->rawName << "in world " << ((PlayerInfo*)(peer->data))->currentWorld << "with IP " << std::hex << peer->address.host << std::dec << " with message " << str.substr(5, cch.length() - 5 - 1) << endl;
	...
	if (((PlayerInfo*)(currentPeer->data))->rawName == pm_user) {
		...
		break;
						...
						delete p.data;
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `4MOD-SB`` `wfrom `6" + name + " `w(in `4UNKNOWN?`w) ** : `# " + str.substr(4, cch.length() - 4 - 1)));
							string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
							BYTE* data = new BYTE[5 + text.length()];
							BYTE zero = 0;
							int type = 3;
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `eVIP-SB`` from `$`6" + name + "```` `w(in `4HIDDEN!`w) ** :`` `# " + str.substr(4, cch.length() - 4 - 1)));
							string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
							BYTE* data = new BYTE[5 + text.length()];
							BYTE zero = 0;
							int type = 3;
							...
							if (((PlayerInfo*)(peer->data))->haveGrowId) {
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `5Super-Broadcast `wfrom `$`2" + name + "```` `w(in `4JAMMED```w) ** :`` `# " + str.substr(4, cch.length() - 4 - 1)));
								string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
								BYTE* data = new BYTE[5 + text.length()];
								BYTE zero = 0;
								int type = 3;
						...
						else if (str.substr(0, 7) == "/item ")
						{
							...
							if (((PlayerInfo*)(peer->data))->haveGrowId) {
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `5Super-Broadcast`` `wfrom `$`2" + name + "```` (in `$" + ((PlayerInfo*)(peer->data))->currentWorld + "``) ** :`` `# " + str.substr(4, cch.length() - 4 - 1)));
								string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
								BYTE* data = new BYTE[5 + text.length()];
								BYTE zero = 0;
								int type = 3;
						...
						else if (str == "/unequip")
						{
							...
							((PlayerInfo*)(peer->data))->skinColor = 2;
							sendClothes(peer);
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`3Your Set has been `5Un-equiped!"));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_label_with_icon|big|Receive Wls|left|242|\nadd_spacer|small|\nadd_label|small|Enter a code to receive GTFX Wls!|left|\nadd_spacer|small|\nadd_url_button||`1Purchase Codes here!|NOFLAGS|https://www.shoppy.gg/@solo_gt|Open Payments?|0|0|\nadd_spacer|small||\nadd_text_input|gtfxcode|CODE:||100|\nend_dialog|chc0|Cancel|Receive!|\nadd_quick_exit|"));
						ENetPacket* packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
						...
						if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 666) {
							...
							WorldInfo* world = getPlyersWorld(peer);
									...
									++currentPeer)
								{
									...
									GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r[MOD-LOGS] `6" + ((PlayerInfo*)(peer->data))->tankIDName + "`r (`6" + ((PlayerInfo*)(peer->data))->displayName + "`r) `4Just nuked world `w" + world->name));
									for (currentPeer = server->peers;
										currentPeer < &server->peers[server->peerCount];
										++currentPeer)
									{
						...
						if (str == "/set") {
							...
							*/
							...
							if (str.substr(0, 6) == "/cancelledcommand ")
							{
					...
					if (!((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						{
							((PlayerInfo*)(event.peer->data))->rawName = "not registered" + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()));
							((PlayerInfo*)(event.peer->data))->displayName = "`w[`4Not Registered`w] " + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()));
							((PlayerInfo*)(event.peer->data))->Chatname = "`w[`4Not Registered`w] " + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()));
						}
						else {
							...
							if (logStatus == -5) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4ERROR?? #13373183, `rYour name is over 18Characters Please Change."));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							if (logStatus == 1) {
								GamePacket pss = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`wServer made by `4Zedd `w+ `4Valkrie`w, For more information Type `5/help `win-game."));
								ENetPacket* packetss = enet_packet_create(pss.data,
									pss.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packetss);
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`2Connecting.. `w[`1Version: `3Beta 1.0 `w- `9Zedd `b+ `9Valkrie`w]``"));
								ENetPacket* packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							case 0:
								...
								if (!((PlayerInfo*)(peer->data))->joinClothesUpdated)
								{
									...
									if (((PlayerInfo*)(peer->data))->isMod == 1) {
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`#Invisible Mode Set!`"));
										ENetPacket* packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
