...
struct PlayerInfo {
	...
	int cloth_ances = 0;
	//store items
	int allowlwing = 0; // legendary wings
	int allowlktana = 0; // legendary katana
	int allowdwing = 0; // legendary knight wing
	int allowled = 0; // dragon of legend
	int allowbot = 0; // robot
	int allowgpic = 0; // golden pickaxe
	int allowepic = 0; // emeraled pickaxe
					   //store items
...
public:
	...
	static int playerRegister(string username, string password, string code);
...
void banlogin(ENetPeer * peer) {
	...
	string dc = "https://discord.gg/2Nc3ysc";
...
int PlayerDB::playerLogin(ENetPeer * peer, string username, string password) {
	...
	if (ifs.is_open()) {
		...
		else if (((PlayerInfo*)(peer->data))->gameversion != "2.997") {

			string text = "action|log\nmsg|`4UPDATE REQUIRED`o : The `$V2.997 `oupdate is now avaliable for your device. Go get it! You'll need that before you can login private server.\n";
			string text3 = "action|logon_fail\n";
...
int PlayerDB::playerRegister(string username, string password, string code) {
	if (username.find(".") != string::npos || username.find(",") != string::npos || username.find("@") != string::npos || username.find("[") != string::npos || username.find("]") != string::npos || username.find("#") != string::npos || username.find("<") != string::npos || username.find(">") != string::npos || username.find(":") != string::npos || username.find("{") != string::npos || username.find("}") != string::npos || username.find("|") != string::npos || username.find("+") != string::npos || username.find("_") != string::npos || username.find("~") != string::npos || username.find("-") != string::npos || username.find("!") != string::npos || username.find("$") != string::npos || username.find("%") != string::npos || username.find("^") != string::npos || username.find("&") != string::npos || username.find("`") != string::npos || username.find("*") != string::npos || username.find("(") != string::npos || username.find(")") != string::npos || username.find("=") != string::npos || username.find("'") != string::npos || username.find(";") != string::npos || username.find(" ") != string::npos || username.find("oflegend") != string::npos || username.find("/") != string::npos) {
		return -77;
	}
	...
	while (updateAnswer)
	{
		...
		if (quest + 300000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
		{
				...
				++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
					continue;
				sendConsole(currentPeer, "`9** GTSA Daily : `3'" + to_string(resultnbr1) + " + " + to_string(resultnbr2) + "'`9 = ? Prize: `2" + to_string(prize) + "`9 (gems) ! `o(/a <answer>).");
				sendSound(currentPeer, "startopia_tool_droid.wav");
			}
			cout << " ** GTSA Daily : '" + to_string(resultnbr1) + " + " + to_string(resultnbr2) + "' = ? Prize: " + to_string(prize) + " gems ! `o(/a answer)." << endl;
		}
	}
...
void sendWorld(ENetPeer * peer, WorldInfo * worldInfo)
{
	...
	if (((PlayerInfo*)(peer->data))->haveGrowId) {
		...
		int bac, han, fac, hai, fee, pan, nec, shi, mas, lvl, ban, pch, skin, mute, gem, anc, exp, join, gemnew, lwing, lktana, dwing, led, bot, ahor, bface, lhair, fhair, mear, gpic, epic, wring, gring, gering, leave;
		bac = j["ClothBack"];
...
void sendWorldCursed(ENetPeer * peer, WorldInfo * worldInfo)
{
	...
	if (((PlayerInfo*)(peer->data))->haveGrowId) {
		...
		int bac, han, fac, hai, fee, pan, nec, shi, mas, lvl, ban, pch, skin, mute, gem, anc, exp, join, gemnew, lwing, lktana, dwing, led, bot, gpic, epic, leave;
		bac = j["ClothBack"];
...
void sendGazette(ENetPeer * peer) {
	...
	string discord = "https://discord.gg/2Nc3ysc";
	...
	if (world->items[x + (y * world->width)].foreground == 6)
	{
		...
		if (((PlayerInfo*)(peer->data))->isIn)
		{
			if (((PlayerInfo*)(peer->data))->haveGrowId) {
				...
				int clothmask = p->cloth_mask;
				int clothances = p->cloth_ances;
				int allowlwing = p->allowlwing;
				int canleave = p->canleave;
				int allowgpic = p->allowgpic;
				int allowepic = p->allowepic;
				int allowlktana = p->allowlktana;
				int allowdwing = p->allowdwing;
				int allowbot = p->allowbot;
				int allowled = p->allowled;
				int level = p->level;
				int skin = p->skinColor;
				int exp = p->blockbroken;
				int entereffect = p->entereffect;
				...
				j["ClothMask"] = clothmask;
				j["ClothAnces"] = clothances;
				j["allowlwing"] = allowlwing;
				j["canleave"] = canleave;
				j["allowgpic"] = allowgpic;
				j["allowepic"] = allowepic;
				j["allowdwing"] = allowdwing;
				j["allowled"] = allowled;
				j["allowbot"] = allowbot;
				j["allowlktana"] = allowlktana;
				j["Level"] = level;
				j["Skin"] = skin;
				j["puncheffect"] = puncheffect;
				j["friends"] = ((PlayerInfo*)(peer->data))->friendinfo;
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	cout << "Growtopia Counch Private Server" << endl;
	enet_initialize();
	...
	*p = 5;*/
	...
	addAdmin("amelia", "aaaaaa", 999);
	addAdmin("null", "nullislit697", 999);
	int itemdathash;
	// load items.dat
	{
			...
			case ENET_EVENT_TYPE_RECEIVE:
			{
				...
				case 2:
				{
					...
					if (cch.find("action|store") == 0)
					{
						if (((PlayerInfo*)(peer->data))->haveGrowId == true)
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`7Welcome to the GTSA store!|left|5956|nadd_label_with_icon|small|`9Legendary items``|left|1790|\n\nadd_button_with_icon|robot||staticBlueFrame|1780|30000|\nadd_button_with_icon|lwings||staticBlueFrame|1784|30000|\nadd_button_with_icon|ledrag||staticBlueFrame|1782|30000|\nadd_button_with_icon|lkatana||staticBlueFrame|2592|30000|\nadd_button_with_icon|ldrag||staticBlueFrame|7734|30000|\nadd_button_with_icon||END_LIST|noflags|0|0||\nadd_spacer|small|\nadd_label_with_icon|small|`7Special items``|left|1900|\nadd_button_with_icon|gpic||staticBlueFrame|1438|70000|\nadd_button_with_icon|epic||staticBlueFrame|4956|70000|\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
					...
					if (cch.find("action|dialog_return") == 0)
					{
								...
								++currentPeer)
							{
								...
								if (isHere(peer, currentPeer))
								{
									...
									if (((PlayerInfo*)(currentPeer->data))->rawName == ((PlayerInfo*)(peer->data))->lastInfo) {
										if (((PlayerInfo*)(currentPeer->data))->rawName == "cmd") continue;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "hadi") continue;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "xxx") continue;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "keta") continue;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "leen") continue;
						...
						if (btn == "cban") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`4Perma ban``|left|276|\nadd_label|small|`oDo you want to use perma ban on this player ?``|left|4|\n\nadd_spacer|small|\n\nadd_button|banbutton|`4YES|0|0|\nadd_button|punish|`5NO|0|0|\nadd_quick_exit"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "ccurse") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`bPerma Curse``|left|278|\nadd_label|small|`oDo you want to use perma curse on this player ?``|left|4|\n\nadd_spacer|small|\n\nadd_button|cursebutton|`4YES|0|0|\nadd_button|punish|`5NO|0|0|\nadd_quick_exit"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "ccurse30") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`b30 days curse``|left|278|\nadd_label|small|`oDo you want to curse this player for 30 days?``|left|4|\n\nadd_spacer|small|\n\nadd_button|curse30|`4YES|0|0|\nadd_button|punish|`5NO|0|0|\nadd_quick_exit"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "cipban") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`4IP-BAN``|left|276|\nadd_label|small|`oDo you want to use ip-ban on this player ?``|left|4|\n\nadd_spacer|small|\n\nadd_button|ipban|`4YES|0|0|\nadd_button|punish|`5NO|0|0|\nadd_quick_exit"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "cban60") {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`560 days ban``|left|276|\nadd_label|small|`oDo you want to ban this player for 60 days ?|left|4|\n\nadd_spacer|small|\n\nadd_button|ban60|`4YES|0|0|\nadd_button|punish|`5NO|0|0|\nadd_quick_exit"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
...
#pragma warning(disable: 4996)
						...
						if (btn == "punish") {
							GamePacket wrench = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|\nadd_spacer|small|\nadd_label_with_icon|big|`1Punish/View|left|4758|\nadd_spacer|small|\nadd_label|small|`4Bans:``|left|4|\n\nadd_spacer|small|\nadd_button|cban|`4Perma ban!|0|0|\nadd_button|cban60|`4Ban for 60 days!|0|0|\nadd_button|cban30|`4Ban for 30 days!|0|0|\nadd_button|cipban|`4IP ban!|0|0|\n\nadd_spacer|small|\nadd_label|small|`5Mutes:``|left|4|\n\nadd_spacer|small|\nadd_button|cmute|`5Perma mute!|0|0|\nadd_button|cmute60|`5Mute for 60 days!|0|0|\nadd_button|cmute30|`5Mute for 30 days!|0|0|\n\nadd_spacer|small|\nadd_label|small|`bCures:``|left|4|\n\nadd_spacer|small|\nadd_button|ccurse|`bPerma curse!|0|0|\nadd_button|ccurse60|`bCurse for 60 days!|0|0|\nadd_button|ccurse30|`bCurse for 30 days!|0|0|\nadd_spacer|small|\nadd_button||Continue|0|0|\nadd_quick_exit"));
									...
									if (((PlayerInfo*)(currentPeer->data))->rawName == ((PlayerInfo*)(peer->data))->lastInfo) {
											...
											++currentPeer)
										{
											...
											if (isHere(peer, currentPeer))
											{
												...
												if (((PlayerInfo*)(currentPeer->data))->rawName == ((PlayerInfo*)(peer->data))->lastInfo) {
													if (((PlayerInfo*)(currentPeer->data))->rawName == "cmd") {
						...
						if (btn == "cmd1") {
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wSecurity Check|left|5814|\nadd_label|small|`oPress TTIKA to continue:``|left|4|\n\nadd_spacer|small|\nadd_button|bot1|`1ALPHT``|0|0|\nadd_button|bot1|`1MODERATOR``|0|0|\nadd_button|bot1|`1CMD``|0|0|\nadd_button|ttika1|`1TTIKA``|0|0|\nadd_button|bot1|`1ARATIC``|0|0|"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						if (btn == "epic") {
							if (((PlayerInfo*)(peer->data))->allowepic == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
							...
							if (((PlayerInfo*)(peer->data))->gem < 70000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Emerald Pickaxe! " + std::to_string(70000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qEmerald Pickaxe`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "gpic") {
							if (((PlayerInfo*)(peer->data))->allowgpic == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (((PlayerInfo*)(peer->data))->gem < 70000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Golden Pickaxe! " + std::to_string(70000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								((PlayerInfo*)(peer->data))->gem = ((PlayerInfo*)(peer->data))->gem - 70000;
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qGolden Pickaxe`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "robot") {
							if (((PlayerInfo*)(peer->data))->allowbot == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (((PlayerInfo*)(peer->data))->gem < 30000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Legendbot-009! " + std::to_string(30000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qLegendbot-009`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "ledrag") {
							if (((PlayerInfo*)(peer->data))->allowled == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (((PlayerInfo*)(peer->data))->gem < 30000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Dragon Of Legend! " + std::to_string(30000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								((PlayerInfo*)(peer->data))->gem = ((PlayerInfo*)(peer->data))->gem - 30000;
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qDragon Of Legend`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "ldrag") {
							if (((PlayerInfo*)(peer->data))->allowdwing == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (((PlayerInfo*)(peer->data))->gem < 30000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Legendary Dragon Knight's Wings! " + std::to_string(30000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								((PlayerInfo*)(peer->data))->gem = ((PlayerInfo*)(peer->data))->gem - 30000;
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qLegendary Dragon Knight's Wings`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "lkatana") {
							if (((PlayerInfo*)(peer->data))->allowlktana == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (((PlayerInfo*)(peer->data))->gem < 30000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Legendary Katana! " + std::to_string(30000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								((PlayerInfo*)(peer->data))->gem = ((PlayerInfo*)(peer->data))->gem - 30000;
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qLegendary Katana`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "lwings") {
							if (((PlayerInfo*)(peer->data))->allowlwing == 1) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You bought this item already"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
							...
							if (((PlayerInfo*)(peer->data))->gem < 30000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to buy Legendary Wings! " + std::to_string(30000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								((PlayerInfo*)(peer->data))->gem = ((PlayerInfo*)(peer->data))->gem - 30000;
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "You bought `qLegendary Wings`0!"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
						...
						if (btn == "ttika1") {
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|`wSecurity Check|left|5814|\nadd_label|small|`oPress ALPHT to continue:``|left|4|\n\nadd_spacer|small|\nadd_button|bot1|`1TTIKA``|0|0|\nadd_button|bot1|`1MODERATOR``|0|0|\nadd_button|bot1|`1ARATIC``|0|0|\nadd_button|bot1|`1CMD``|0|0|\nadd_button|alpht1|`1ALPHT``|0|0|"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
								...
								++currentPeer)
							{
								...
								if (isHere(peer, currentPeer))
								{
									...
									if (((PlayerInfo*)(currentPeer->data))->rawName == ((PlayerInfo*)(peer->data))->lastInfo) {
										if (!canSB(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))break;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "cmd") continue;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "alpht") continue;
										if (((PlayerInfo*)(currentPeer->data))->rawName == "ttika") continue;
						...
						if (isFindDialog && btn.substr(0, 4) == "tool") {
							...
							if (Id == 1784)
							{
								if (((PlayerInfo*)(peer->data))->allowlwing == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
								...
								else	if (((PlayerInfo*)(peer->data))->allowlwing == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
							...
							{
								if (((PlayerInfo*)(peer->data))->allowlktana == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
								...
								else	if (((PlayerInfo*)(peer->data))->allowlktana == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
							...
							{
								if (((PlayerInfo*)(peer->data))->allowdwing == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
								...
								else	if (((PlayerInfo*)(peer->data))->allowdwing == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
							...
							{
								if (((PlayerInfo*)(peer->data))->allowbot == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
								...
								else	if (((PlayerInfo*)(peer->data))->allowbot == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
							...
							{
								if (((PlayerInfo*)(peer->data))->allowled == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
								...
								else	if (((PlayerInfo*)(peer->data))->allowled == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
							...
							{
								if (((PlayerInfo*)(peer->data))->allowgpic == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
								...
								else	if (((PlayerInfo*)(peer->data))->allowgpic == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
							...
							{
								if (((PlayerInfo*)(peer->data))->allowepic == 0) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`4You Should Buy This item from the shop first!!!"));
									ENetPacket* packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
								...
								else	if (((PlayerInfo*)(peer->data))->allowepic == 1) {
									int Id = atoi(btn.substr(4, btn.length() - 4).c_str());
					...
					SKIPFind:;
						...
						if (isRegisterDialog) {
							...
							int regState = PlayerDB::playerRegister(username, password, code);
							if (regState == 1) {
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Sorry! We couldn't complete your registration, Because u used bad characters in your name!``"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
					...
					if (cch.find("text|") != std::string::npos) {
						...
						if (str == "/noclip")
						{
							if (((PlayerInfo*)(peer->data))->rawName == world->owner || isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) {
								...
								if (world->noclip) {
									...
									world->noclip = false;
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Sorry, you cant use noclip here because the owner disabled it"));
									ENetPacket * packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
								...
								if (getAdminLevel(((PlayerInfo*)(currentPeer->data))->rawName, ((PlayerInfo*)(currentPeer->data))->tankIDPass) > 4) {
									...
									else
									{
										...
										if (getAdminLevel(((PlayerInfo*)(currentPeer->data))->rawName, ((PlayerInfo*)(currentPeer->data))->tankIDPass) == 999) {
											...
											if (((PlayerInfo*)(currentPeer->data))->rawName == "hadi")
											{
							...
							if ((str.substr(6, cch.length() - 6 - 1) == "cmd") || (str.substr(6, cch.length() - 6 - 1) == "ttika") || (str.substr(6, cch.length() - 6 - 1) == "alpht")) continue;
							...
							sendConsoleMsg(peer, "`6" + str);
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Dont use any hack client, Dont use auto clicker, Dont be racist, Dont msg mods for free gems, Dont scam people gems")); /*From here you can change the rules*/
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999 || getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 5) {
								string x;
									...
									++currentPeer)
								{
									...
									if (getAdminLevel(((PlayerInfo*)(currentPeer->data))->rawName, ((PlayerInfo*)(currentPeer->data))->tankIDPass) < 5)
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->tankIDName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Supported commands are: /mods, /help, /noclip, /unnoclip, /find, /team id, /color number, /who, /count, /sb message, /jsb message, /alt, /radio, /rules, /news, /p id, /resetset, /clear, /access (name), /unaccess (name), /msg, /r ,/pull , /kick, /kickall , /giveworld <name>, /rgo ,/time ,/wl ,/clearinv, /changepass, /a (answer)                                                         `1SERVER-CREATOR COMMANDS :  /ssb, /vsb, /msb, /nick, /asb, /dev, /invis, /vis, /wizard, /ringmaster, /mute (name), /vip, /unvip, /mod, /unmod ,/scn, /unscn, /ban (name), /warn (name), /hnick (to hide your nick), /snick (to show your nick), /spawn (item id), /unban (name), /unmute (name), /summon (name), /give (real gems), /gem (fake gems), /freeze (name), /warp (world), /warpto (name), /destructo, /xp, /level, /fban (name), /wsb, /find (name), /send (world name), /ipban (name), /nuke (use it again to unnuke), /curse (name), /warppl (name), /reset, /gsm, /adm (name - to add mods), /ads (name - to add sc), /dem (name - to demote mod or sc) /uncurse (name-- use it for uncurse the perma curse), /curse60 (name), /curse30 (name), /tuncurse (name-- use it to uncurse timed curse), /mute60 (name), /mute30 (name), /tunmute (name-- use it to unmute time mute), /tunban (name-- use it to unban time ban), /superclearworld"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							else if (getAdminLevel(((PlayerInfo*)(peer->data))->tankIDName, ((PlayerInfo*)(peer->data))->tankIDPass) == 5) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Supported commands are: /mods, /help, /noclip, /unnoclip, /find, /team id, /color number, /who, /count, /sb message, /jsb message, /alt, /radio, /rules, /news, /p id, /resetset, /clear, /access (name), /unaccess (name), /msg, /r ,/pull , /kick, /kickall , /giveworld <name>, /rgo ,/time ,/wl ,/clearinv, /changepass, /a (answer)                                                         `5Moderators Commands Are:``/nick, /asb, /vsb /msb, /dev, /invis, /vis, /wizard, /ringmaster, /mute (name), /vip, /unvip, /mod, /unmod, /ban (name), /warn (name), /hnick (to hide your nick) ,/snick (to show your nick), /unban (name-- use it for unaban the perma ban) ,/unmute (name-- use it to unmute perma mute),  /summon (name), /spawn (item id), /freeze (name), /warp (world), /warpto (name) ,/destructo, /fban (name), /wsb, /find (name), /send (world name), /ipban (name), /nuke (use it again to unnuke), /block (id), /curse (name), /warppl (name), /copyset (user), /uncurse (name-- use it for uncurse the perma curse), /curse60 (name), /curse30 (name), /tuncurse (name-- use it to uncurse timed curse), /mute60 (name), /mute30 (name), /tunmute (name-- use it to unmute time mute), /tunban (name-- use it to unban time ban)"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							else if (getAdminLevel(((PlayerInfo*)(peer->data))->tankIDName, ((PlayerInfo*)(peer->data))->tankIDPass) == 4) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Supported commands are: /mods, /help, /noclip, /unnoclip, /find, /team id, /color number, /who, /count, /sb message, /jsb message, /alt, /radio, /rules, /news, /p id, /resetset, /clear, /access (name), /unaccess (name), /msg, /r ,/pull , /kick, /kickall , /giveworld <name>, /rgo ,/time ,/wl ,/clearinv, /changepass, /a (answer)                                                         `rVIP Commands Are:``/vip, /unvip, /vsb, /find (name), /hnick, /snick, /warp, /warpto (name), /block (id), /copyset (name"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
							...
							else if (getAdminLevel(((PlayerInfo*)(peer->data))->tankIDName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Supported commands are: /mods, /help, /noclip, /unnoclip, /find, /team id, /color number, /who, /count, /sb message, /jsb message, /alt, /radio, /rules, /news, /p id, /resetset, /clear,  /msg, /r ,/pull , /kick, /kickall , /giveworld <name>, /rgo ,/time ,/wl ,/clearinv, /changepass, /online, /cry, /access (name), /unaccess (name), /a (answer)"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						else if (str == "/cry")
						{
						...
						else if (str.substr(0, 8) == "/access ") { // access system by cmd
						...
						else if (str.substr(0, 10) == "/unaccess ") { // unaccess system by cmd
							...
							if ((str.substr(8, cch.length() - 8 - 1) == "cmd") || (str.substr(8, cch.length() - 8 - 1) == "ttika") || (str.substr(8, cch.length() - 8 - 1) == "alpht")) continue;
...
#pragma warning(disable: 4996)
							...
							if ((str.substr(9, cch.length() - 9 - 1) == "cmd") || (str.substr(9, cch.length() - 9 - 1) == "ttika") || (str.substr(9, cch.length() - 9 - 1) == "alpht")) continue;
...
#pragma warning(disable: 4996)
							...
							if (imie == hasil) {
									...
									++currentPeer)
								{
									if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
										continue;
									sendConsole(currentPeer, "`9** GTSA Daily: (party) Math Event Winner is `w" + nama + "`9!");
...
#pragma warning(disable: 4996)
							...
							if (((PlayerInfo*)(peer->data))->gem < 3000) {
								GamePacket p8 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "You don't not have enought gem to clear the world! " + std::to_string(3000 - ((PlayerInfo*)(peer->data))->gem) + "  more gem"));
								ENetPacket * packet8 = enet_packet_create(p8.data,
									p8.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet8);
								...
								if (((PlayerInfo*)(peer->data))->rawName == world->owner || isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass)) {
									...
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#[ `$You Cleared the world `#]"));
									ENetPacket * packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
						...
						else if (str.substr(0, 7) == "/curse ") { // code by cmd
							if (!canSB(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))break;
							if (str.substr(7, cch.length() - 7 - 1) == "") continue;
							if ((str.substr(7, cch.length() - 7 - 1) == "cmd") || (str.substr(7, cch.length() - 7 - 1) == "ttika") || (str.substr(7, cch.length() - 7 - 1) == "alpht")) continue;
							if (((PlayerInfo*)(peer->data))->rawName == str.substr(7, cch.length() - 7 - 1)) continue;
						...
						else if (str.substr(0, 7) == "/spawn ")
						{
							...
							if ((str.substr(5, cch.length() - 5 - 1) == "cmd") || (str.substr(5, cch.length() - 5 - 1) == "ttika") || (str.substr(5, cch.length() - 5 - 1) == "alpht")) continue;
						...
						cout << "Server operator " << ((PlayerInfo*)(peer->data))->rawName << " has ipbanned " << str.substr(8, cch.length() - 8 - 1) << "." << endl;

						ENetPeer * currentPeer;

						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `$The Ancient Ones `ohave `4mac-banned `2" + str.substr(8, cch.length() - 8 - 1) + " `#** `o(`4/rules `oto see the rules!)"));
						for (currentPeer = server->peers;
						currentPeer < &server->peers[server->peerCount];
						++currentPeer)
						{
						...
						if (((PlayerInfo*)(currentPeer->data))->rawName == str.substr(8, cch.length() - 8 - 1)) {
						if (((PlayerInfo*)(currentPeer->data))->haveGrowId == false) continue;
						GamePacket ps = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`#** `$The Ancient Ones `ohave used `#Mac-Ban `oon `2" + str.substr(8, cch.length() - 8 - 1) + "`o! `#**"));
						ENetPacket * packet = enet_packet_create(ps.data,
						ps.len,
						ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(currentPeer, 0, packet);
						...
						string macban = "";
						std::ifstream ifs("macban.json");
						ENetPeer * peer123 = currentPeer;
						//((PlayerInfo*)(event.peer->data))->macaddress = act;
						string mac = ((PlayerInfo*)(peer123->data))->macaddress;
						if (ifs.is_open()) {

						json j3;
						ifs >> j3;
						macban = j3["mac"];
						macban = macban.append("|" + mac + "|");
						}
						std::ofstream od("macban.json");
						if (od.is_open()) {

						}

						std::ofstream o("macban.json");
						if (!o.is_open()) {
						cout << GetLastError() << endl;
						_getch();
						}
						...
						int allowgpic = p->allowgpic;
						int allowepic = p->allowepic;
						int allowgring = p->allowgring;
						int allowwring = p->allowwring;
						int allowgering = p->allowgering;
						int allowahor = p->allowahor;
						int allowbface = p->allowbface;
						int allowlhair = p->allowlhair;
						int allowfhair = p->allowfhair;
						int allowmear = p->allowmear;
						int allowdwing = p->allowdwing;
						int allowled = p->allowled;
						int allowbot = p->allowbot;
						int allowlktana = p->allowlktana;
						...
						j["allowlwing"] = allowlwing;
						j["canleave"] = canleave;
						j["allowwring"] = allowwring;
						j["allowgring"] = allowgring;
						j["allowgering"] = allowgering;
						j["allowgpic"] = allowgpic;
						j["allowepic"] = allowepic;
						j["allowahor"] = allowahor;
						j["allowbface"] = allowbface;
						j["allowfhair"] = allowfhair;
						j["allowlhair"] = allowlhair;
						j["allowmear"] = allowmear;
						j["allowdwing"] = allowdwing;
						j["allowled"] = allowled;
						j["allowbot"] = allowbot;
						j["ClothAnces"] = clothances;
						...
						else if (str == "/wizard")
						{
							...
							((PlayerInfo*)(peer->data))->cloth_necklace = 0;
							sendClothes(peer);
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`Wizard Set Mod has been Enabled! "));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						else if (str == "/ringmaster")
						{
							...
							((PlayerInfo*)(peer->data))->cloth_pants = 0;
							((PlayerInfo*)(peer->data))->cloth_feet = 0;
							((PlayerInfo*)(peer->data))->cloth_face = 1900;
							((PlayerInfo*)(peer->data))->cloth_hand = 0;
							((PlayerInfo*)(peer->data))->cloth_back = 0;
							((PlayerInfo*)(peer->data))->cloth_mask = 0;
							((PlayerInfo*)(peer->data))->cloth_necklace = 0;
							sendClothes(peer);
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`^RingMaster Set Mod has been Enabled! "));
							ENetPacket* packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
					...
					if (!((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						{
							((PlayerInfo*)(event.peer->data))->rawName = "NoID" + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()));
							((PlayerInfo*)(event.peer->data))->displayName = "NoID " + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()));
							((PlayerInfo*)(event.peer->data))->Chatname = "NoID " + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length() > 15 ? 15 : ((PlayerInfo*)(event.peer->data))->requestedName.length()));
						}
						else {
							...
							if (logStatus == 1) {
								GamePacket pss = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4if you cant join `4please login to real gt first"));
						...
						{
							(((PlayerInfo*)(event.peer->data))->displayName = "`e" + (((PlayerInfo*)(event.peer->data))->rawName));
							(((PlayerInfo*)(event.peer->data))->Chatname = "`e" + (((PlayerInfo*)(event.peer->data))->rawName));
						...
						if (((PlayerInfo*)(event.peer->data))->rawName == "hadi")
						{
					...
					if (pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						if (((PlayerInfo*)(peer->data))->haveGrowId) {
							...
							int bac, han, fac, hai, fee, pan, nec, shi, mas, lvl, ban, pch, skin, mute, gem, enter, anc, exp, join, gemnew, lwing, lktana, dwing, led, bot, gpic, epic, leave;
							bac = j["ClothBack"];
